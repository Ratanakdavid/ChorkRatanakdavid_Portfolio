...
export default function MobileNav({ navOpen }: MobileNavProps) {
  ...
  return (
    <>
    {navOpen && <div className={`fixed inset-0 transform right-0 z-50 bg-black opacity-30 w-full
    h-screen transition-all duration-500 ${showMobileNav}`}></div>}
    ...
    bg-slate-800 space-y-1 z-80 right-0 top-0 ${showMobileNav}`}>
        {navLinks.map((link) => {
            return (
                <li key={link.url}>
                    <Link href={link.url} className="flex justify-center items-center
                    text-xl font-medium text-white py-4 px-6
